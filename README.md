...
## Status
Turned out 2017 was quite short and 2018 doesn't seem to be any longer so this is still heavily work in progress. But there is progress. A2DP sink to I2S is working. Implementation based on the ESP32 examples provided by *Espressif* was pretty straight forward.
I initially used the ESP32's integrated DAC which sounded not so well. So I got me some cheap I2S DAC from China and initial testing with some old scrap laptop speakers sounded way better using the dedicated DAC. Next step is to actually get the sound out of the car radio which requires it to enable the CDC input. This will be done talking SPI to car radio. On my device the respective pins are names *DATA-IN* and *CLK* which are *MOSI* and *CLOCK* of the SPI. *MISO* is not existing because the CDC uses a different interface to talk to the car radio. It's probably the *NEC* protocol which is commonly used in infrared remote controls. Nice thing about that is that there again is an exmaple for ESP32 using the *RMT* peripheral to receive *NEC* modulated signals. Lucky me. I've been waiting for a logic level converter the past few days which has just arrived. So tinkering can go on now.
